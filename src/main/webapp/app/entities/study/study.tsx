...
export const Study = (props: IStudyProps) => {
  ...
  const { studyList, match, loading, totalItems } = props;
  return (
    <div>
      <h2 id="study-heading">
        Studies
      ...
      <div className="table-responsive">
        {studyList && studyList.length > 0 ? (
            ...
            <thead>
              <tr>
                ...
                <th className="hand" onClick={sort('resolution')}>
                  Resolution <FontAwesomeIcon icon="sort" />
                </th>
                <th className="hand" onClick={sort('legislation')}>
                  Legislation <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
                ...
                <tr key={`entity-${i}`}>
                  ...
                  <td className="text-right">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        tag={Link}
                        to={`${match.url}/${study.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      >
                      ...
                      <Button
                        tag={Link}
                        to={`${match.url}/${study.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="danger"
                        size="sm"
                      >
          ...
          !loading && <div className="alert alert-warning">No Studies found</div>
        )}
      </div>
      {props.totalItems ? (
