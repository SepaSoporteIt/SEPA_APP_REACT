...
export interface IVisitsProps extends StateProps, DispatchProps, RouteComponentProps<{ url: string }> {}

export const Visits = (props: IVisitsProps) => {
  ...
  const { visitsList, match, loading, totalItems } = props;
  return (
    <div>
      <h2 id="visits-heading">
        <Translate contentKey="sepaApp.visits.home.title">Visits</Translate>
        <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
          <FontAwesomeIcon icon="plus" />
          &nbsp;
          <Translate contentKey="sepaApp.visits.home.createLabel">Create new Visits</Translate>
        </Link>
      </h2>
        ...
        {visitsList && visitsList.length > 0 ? (
          <Table responsive>
            ...
            <tbody>
              {visitsList.map((visits, i) => (
                <tr key={`entity-${i}`}>
                  <td>
                    <Button tag={Link} to={`${match.url}/${visits.id}`} color="link" size="sm">
                      {visits.id}
                    </Button>
                  </td>
                  ...
                  <td className="text-right">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        tag={Link}
                        to={`${match.url}/${visits.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      >
                      ...
                      <Button
                        tag={Link}
                        to={`${match.url}/${visits.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="danger"
                        size="sm"
                      >
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="sepaApp.visits.home.notFound">No Visits found</Translate>
            </div>
          )
        )}
      </div>
