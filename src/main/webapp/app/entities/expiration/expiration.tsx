...
import { getEntities, reset } from './expiration.reducer';
...
export const Expiration = (props: IExpirationProps) => {
  ...
  const { expirationList, match, loading } = props;
  return (
    <div>
      ...
      <div className="table-responsive">
        ...
        >
          {expirationList && expirationList.length > 0 ? (
              ...
              <tbody>
                {expirationList.map((expiration, i) => (
                  <tr key={`entity-${i}`}>
                    ...
                    <td>{expiration.employee ? <Link to={`employee/${expiration.employee.id}`}>{expiration.employee.name}</Link> : ''}</td>
            ...
            !loading && <div className="alert alert-warning">No Expirations found</div>
          )}
        </InfiniteScroll>
      </div>
    </div>
