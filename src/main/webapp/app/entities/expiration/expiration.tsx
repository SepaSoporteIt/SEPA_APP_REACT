            ...
            <tbody>
              {expirationList.map((expiration, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>
                    {expiration.startDate ? <TextFormat type="date" value={expiration.startDate} format={APP_LOCAL_DATE_FORMAT} /> : null}
                  </td>
                  <td>
                    {expiration.endDate ? <TextFormat type="date" value={expiration.endDate} format={APP_LOCAL_DATE_FORMAT} /> : null}
                  </td>
                  <td>
                  ...
                  <td>{expiration.isCompleted ? 'true' : 'false'}</td>
                  <td>{expiration.company ? <Link to={`company/${expiration.company.id}`}>{expiration.company.id}</Link> : ''}</td>
                  <td>{expiration.employee ? <Link to={`employee/${expiration.employee.id}`}>{expiration.employee.id}</Link> : ''}</td>
                  <td>{expiration.study ? <Link to={`study/${expiration.study.id}`}>{expiration.study.id}</Link> : ''}</td>
