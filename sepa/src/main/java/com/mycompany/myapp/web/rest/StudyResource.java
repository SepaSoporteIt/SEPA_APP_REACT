...
public class StudyResource {
    ...
    public ResponseEntity<Void> deleteStudy(@PathVariable Long id) {
        ...
        studySearchRepository.deleteById(id);
        return ResponseEntity.noContent().headers(HeaderUtil.createEntityDeletionAlert(applicationName, true, ENTITY_NAME, id.toString())).build();
    }

    /**
     * {@code SEARCH  /_search/studies?query=:query} : search for the study corresponding
     * to the query.
     *
    ...
    public ResponseEntity<List<Study>> searchStudies(@RequestParam String query, Pageable pageable) {
        ...
        Page<Study> page = studySearchRepository.search(queryStringQuery(query), pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
}
