...
export const ACTION_TYPES = {
  SEARCH_STUDIES: 'study/SEARCH_STUDIES',
...
export default (state: StudyState = initialState, action): StudyState => {
  switch (action.type) {
    case REQUEST(ACTION_TYPES.SEARCH_STUDIES):
    ...
    case FAILURE(ACTION_TYPES.SEARCH_STUDIES):
    ...
    case SUCCESS(ACTION_TYPES.SEARCH_STUDIES):
...
const apiSearchUrl = 'api/_search/studies';

// Actions

export const getSearchEntities: ICrudSearchAction<IStudy> = (query, page, size, sort) => ({
  type: ACTION_TYPES.SEARCH_STUDIES,
  payload: axios.get<IStudy>(`${apiSearchUrl}?query=${query}${sort ? `&page=${page}&size=${size}&sort=${sort}` : ''}`)
});
